...
{   
     ...
     $rproductname=$row['productname'];
     $rcost=$row['cost'];
     $rcosmetics=$row['cosmetics'];
   ...
   if(!empty($_SESSION['id']))	
   {
    ...
    if(!empty($chk)) 
	{
		if(empty($img_name))
	       ...
	       {
		          $paymentmode=implode(",",$chk);
	              if(mysqli_query($con,"update  tbproduct  set productname='$product_name',cost='$cost',cosmetics='$cosmetics',paymentmode='$paymentmode' where product_id='$product_id'"))
	   
	                {
						header("location:update_product.php?product_id=$product_id");
		              //echo "update with old pic";
		              //move_uploaded_file($_FILES['product_pic']['tmp_name'],"../image/Salon/$em/productimages/".$_FILES['product_pic']['name']);
	                 }
	              else{
		               echo "failed";
	                  }
	     ...
	     else
		 {
			     ...
			     echo $paymentmode."<br>";
			     echo $img_name."<br>";
	             if(mysqli_query($con,"update  tbproduct  set productname='$product_name',cost='$cost',cosmetics='$cosmetics',paymentmode='$paymentmode', fldimage='$img_name' where product_id='$product_id'"))
	
	                {
		             echo "update with new pic";
		             move_uploaded_file($_FILES['product_pic']['tmp_name'],"image/Salon/$em/productimages/".$_FILES['product_pic']['name']);
	                 unlink("image/Salon/$em/productimages/$rfldimageold");
					 header("location:update_product.php?product_id=$product_id");
					}
				 else
				 {
                  ...
                  <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     ...
                                     <label for="product_name">Product Name:</label>
                                            <input type="text" class="form-control" id="product_name" value="<?php if(isset($rproductname)) { echo $rproductname;}?>" placeholder="Enter Product Name" name="product_name" required>
                                     </div>
	                                 ...
	                                 <div class="form-group"><!--cuisines-->
                                            ...
                                            <input type="text" class="form-control" id="cosmetics" value="<?php if(isset($rcosmetics)) { echo $rcosmetics;}?>" placeholder="Enter Cosmetics" name="cosmetics" required>
                                    </div>
